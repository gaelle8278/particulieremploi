...
$titleDoc="CONTRAT DE TRAVAIL A DUREE INDETERMINEE (CDI)";
...
$table->addRow(200);
$table->addCell(\PhpOffice\PhpWord\Shared\Converter::cmToTwip(21), $titleTableCellStyle)->addText($titleDoc, $titleTableFontStyle, $cellHCentered);
...
$section->addText("À défaut d'accord entre les parties, la date des congés est fixée par l'employeur, sous réserve du respect d'un délai de "
 . "prévenance minimal de deux mois.",
        null, 'pJustify');
...
$section->addText("Clauses particulières : ",
        $subtitleFontStyleName);
$section->addText("- Congés liés aux contraintes professionnelles de l'employeur, ");
$section->addText("- Evolution possible des tâches, des horaires,");
$section->addText("- Logement de fonction");
$section->addText("- Etc.");
$section->addTextBreak(1);
...
$table->addRow(300);
$table->addCell(\PhpOffice\PhpWord\Shared\Converter::cmToTwip(10), $titleTableCellStyle)->addText("SALARIE DU PARTICULIER EMPLOYEUR", $subtitleTableFontStyle, $cellHCentered);
$section->addTextBreak(1, null, $noSpace);
$section->addText("ATTENTION : ce modèle n’est pas adapté si vous résidez dans les DOM. ", array('color'=>'428ABD', 'bold'=>true, 'size'=>14));
...
$textrun->addText("identifier la nature de l'emploi, ", $colored);
$textrun->addText("de votre salarié(e), il vous est préconisé de vous rendre sur le simulateur prévu à cet effet et de suivre les indications données : ");
$textrun->addLink("http://www.simulateur-emploisalarieduparticulieremployeur.fr", "http://www.simulateur-emploisalarieduparticulieremployeur.fr", $linkFontStyleName);
$section->addText("Une fois l’emploi identifié vous pouvez vous rendre sur notre générateur de contrat de travail qui permettra de joindre automatiquement le descriptif de votre emploi au contrat.", null, 'pJustify');
...
$section->addText("NB : pensez à joindre en annexe du contrat le descriptif de l'emploi-repère ou rendez-vous sur notre générateur de contrat de travail qui permet de joindre automatiquement ce descriptif.", $bold);
...
$textrun->addText("S'il y a ");
$textrun->addText("plusieurs activités principales de durée équivalente,", $underlined);
$textrun->addText(" le particulier employeur retient l'ensemble des emplois-repères correspondant et l'échelle la plus élevée de ces emplois-repères.");
...
$textrun->addText("S'il y a ");
$textrun->addText("des activités complémentaires ", $underlined);
$textrun->addText("aux activités de l'emploi-repère retenu, (c'est-à-dire des activités qui ne figurent pas dans la liste de l'emploi-repère retenu) le particulier employeur les liste dans le contrat de travail.");
$section->addText("Les activités complémentaires ne donnent pas lieu à majoration de salaire sauf négociation entre les parties.", null, 'pJustify'); 
...
//$section->addTextBreak(2, null, $noSpace);
...
$section->addText("En cas de durée de travail hebdomadaire régulière (temps de travail minimum régulier chaque semaine), il est obligatoire de mensualiser le salaire :", null, 'pJustify');
$section->addText("Salaire mensualisé = durée mensuelle du travail** X taux horaire", null, 'pJustify');
$section->addText("Durée mensuelle du travail** = durée hebdomadaire de travail X 52 semaines / 12 mois (arrondir, le cas échéant, à l’entier supérieur en cas de déclaration via le Cesu ou le Pajemploi).", null,'pJustify');
...
$section->addText("Cas particulier de l'année d'embauche.");
$section->addText("La première année d'embauche jusqu'au 31 mai, le(a) salarié(e) ne peut pas prendre de congés payés, la période de référence ne courant que du 1er juin de l'année N au 31 mai de l'année N+1.", null, 'pJustify');
