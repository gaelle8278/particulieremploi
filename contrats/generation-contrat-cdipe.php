<?php
/**
* Génération du PDF pour le contrat CDI salarié particulier employeur
*/
...
/*echo mb_detect_encoding($_POST['adresseEmp']);
echo 'tototototo';
$value=$_POST['adresseEmp'];
$value2=mb_convert_encoding($value,'ISO-8859-1','UTF-8');
echo "<br>".mb_detect_encoding($value2);
echo "<br><br>".$value2;
echo "<br>".mb_detect_encoding($value);
echo "<br>".mb_detect_encoding(utf8_decode($_POST['adresseEmp']));
echo "<br>".mb_detect_encoding(htmlspecialchars($_POST['adresseEmp'],ENT_QUOTES,'ISO-8859-1'));
echo "<br>".$_POST['adresseEmp'];
echo "<br>".htmlspecialchars($_POST['adresseEmp']);
...
$pdf->ln(5);
$textPE="<p>Toute suspension du contrat qui se produirait pendant la période d'essai (maladie, congés, 
...
$pdf->writeHTMLCell(25, 25, 150,'',"",1, 1,0,1,'C');
//4ème ligne
$pdf->writeHTMLCell(30, 25, '','',"<br><br><br>Total travail <br>effectif",1, 0,0,1,'C');
$pdf->writeHTMLCell(25, 25, 50,'',"", 1, 0,0,1,'C');
$pdf->writeHTMLCell(25, 25, 75,'', "",1, 0,0,1,'C');
$pdf->writeHTMLCell(25, 25, 100,'', "",1, 0,0,1,'C');
$pdf->writeHTMLCell(25, 25, 125,'', "",1, 0,0,1,'C');
...
$textConges="Le(la) salarié(e) bénéficiera des congés payés définis à l'article 16 de la convention collective.<br>"
 . "À défaut d'accord entre les parties, la date des congés est fixée par l'employeur, sous réserve du respect d'un délai de "
 . "prévenance minimal de deux mois. <br> ";
...
$pdf->ln(1);
$pdf->SetTextColor(0,0,0);
$pdf->writeHTMLCell('','','','',"- Congés liés aux contraintes professionnelles de l'employeur, ",0,1,false,true,'J',true, false);
$pdf->ln(1);
$pdf->writeHTMLCell('','','','',"- Evolution possible des tâches, des horaires,",0,1,false,true,'J',true, false);
$pdf->ln(1);
$pdf->writeHTMLCell('','','','',"- Logement de fonction",0,1,false,true,'J',true, false);
$pdf->ln(1);
$pdf->writeHTMLCell('','','','',"- Etc.",0,1,false,true,'J',true, false);
$pdf->ln(5);
...
$pdf->writeHTMLCell(100,'',$xpos,'',"SALARIE DU PARTICULIER EMPLOYEUR",0,1,true,true,'C');
$pdf->ln(2);
$pdf->SetTextColor(67, 139, 190);
$pdf->writeHTMLCell('', '', '', '', "ATTENTION : ce modèle n’est pas adapté si vous résidez dans les DOM. ", 0, 1, 0, true, 'L');
$pdf->ln(4);


$pdf->SetTextColor(0,0,0);
$pdf->SetFont('times', '', 11, '', true);
$texteColGauche="<b><span style=\"color:#EF8914;\">Période d'essai :</span></b><br>
2 mois maximum. <br>
...
Pour <span style=\"color:#EF8914;\">identifier la nature de l'emploi,</span> de votre salarié(e), 
il vous est préconisé de vous rendre sur le simulateur prévu à cet effet et de suivre les indications données :
<a href=\"http://www.simulateur-emploisalarieduparticulieremployeur.fr/\">http://www.simulateur-emploisalarieduparticulieremployeur.fr/</a>
<br> 
<br>
Une fois l’emploi identifié vous pouvez vous rendre sur notre générateur de contrat de travail 
qui permettra de joindre automatiquement le descriptif de votre emploi au contrat.<br>
<br>
<b><span style=\"color:#EF8914;\">Nature de l'emploi :</span></b><br>
...
$texteColDroite="&bull; <b>Domaine « Environnement externe »</b> (la réalisation d’activités de petits travaux de bricolage, petits travaux de jardinage, gardiennage)  (cliquez <a href=\"http://www.fepem.fr/documents/10163/1274052/ANNEXE+5+domaine+environnement+externe_Avril2016.pdf\">ici</a>).<br>
<br>
...
<br>
<b>NB : pensez à joindre en annexe du contrat le descriptif de l'emploi-repère ou rendez-vous 
sur notre générateur de contrat de travail qui permet de joindre automatiquement ce descriptif.</b>";
...
$texteColGauche="<b><span style=\"color:#EF8914;\">Durée du travail hebdomadaire :</span></b><br> 
...
Il convient d'indiquer le salaire horaire brut minimum conventionnel correspondant au niveau de l'échelle de l'emploi-repère retenu. <br>
<br>
Il n'est pas obligatoire de mentionner les salaires nets.<br>
<br>
En cas de durée de travail hebdomadaire régulière (temps de travail minimum régulier chaque semaine), 
il est obligatoire de mensualiser le salaire :<br>
Salaire mensualisé = durée mensuelle du travail** X taux horaire<br>
Durée mensuelle du travail** = durée hebdomadaire de travail X 52 semaines / 12 mois 
(arrondir, le cas échéant, à l’entier supérieur en cas de déclaration via le Cesu ou le Pajemploi).<br>
<br>
...
Si le salarié travaille 32h ou plus/mois (sauf en cas de volet social Cesu papier), les congés payés sont rémunérés 
quand ils sont pris sauf accord au contrat entre les parties prévoyant que le salaire soit majoré de 10% au titre des congés payés.
";
$y = $pdf->getY();
$pdf->writeHTMLCell(86, '', '', $y, $texteColGauche, 0, 0, 0, true, 'J', true);
$pdf->writeHTMLCell(5, '', '', '', "", 0, 0, 0, true, 'J', true);
$texteColDroite1="<u>Frais d'abonnement aux transports en 
commun</u><br>
...
Cas particulier de l'année d'embauche.<br>
La première année d'embauche jusqu'au 31 mai, le(a) salarié(e) ne peut pas prendre de 
congés payés, la période de référence ne courant que du 1er juin de l'année N au 31 mai de l'année N+1. <br>
Il conviendra de prévoir les modalités de prise de congés pour cette année ainsi que le mode de rémunération (sans solde ou par anticipation).<br>
<br>
<br>";
...
Nos conseillers vous répondent aussi sur votre Espace Particulier Employeur accessible <a href=\"http://reseau.fepem.fr\">ici</a>.
<br><br>
...
$pdf->writeHTMLCell(86,'', 112, '', $textecolDroite2, 0, 1, 0, true, 'J', true);
